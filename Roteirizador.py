...
if modo == "Por semana":
    ...
    periodo = st.date_input(
        ...
        value=(st.session_state.parametros["inicio"], st.session_state.parametros["inicio"] + timedelta(days=4)),
...
if executar:
    ...
    elif modo == "Por intervalo de datas" and periodo and isinstance(periodo, tuple):
        inicio_sel, fim_sel = periodo
        agenda_filtrada = {
            sem: dias for sem, dias in agenda.items()
            if inicio_sel <= datetime.datetime.strptime(sem.split("(")[1].split(" - ")[0].strip(), "%d/%m/%Y").date() <= fim_sel
        }
    else:
        agenda_filtrada = {}
