...
DEFAULTS = {
    ...
    "semanas": 6,
    "inicio": datetime.date(2025, 12, 24),  # Semana 1 sempre começa hoje
    "rota": None,
...
def gerar_agenda(inicio, semanas, clientes, capacidade_por_dia, rota=None,
    ...
    # Gerar semanas reais com base no calendário
    for sidx in range(semanas):
        ...
        for c in base:
            ...
            if freq == 7:  # semanal
                ...
                if semana_inicio.day <= 7:  # primeira semana do mês
                    agenda[semana_id][dia].append(c)
            else:
...
semana_escolhida = st.selectbox("Selecione a semana para visualizar", semanas_disponiveis)

if semana_escolhida:
    dados = []
    # Extrair datas reais da semana escolhida
    periodo = semana_escolhida.split("(")[1].split(")")[0]
    semana_inicio_str, semana_fim_str = periodo.split(" - ")
...
# -------------------------------
# (restante do seu código continua igual: redistribuição, alertas etc.)
# -------------------------------

# -------------------------------
# Redistribuição balanceada
# -------------------------------
def redistribuir_balanceado(agenda, capacidade_por_dia, permitir_mover_semanal=False, preservar_dia_semana=False):
    """
    Balanceia as cargas dentro de cada semana movendo clientes de dias sobrecarregados
    para dias com folga até aproximar todos da média/limite.
    Prioridade de movimento: freq 30 -> 14 -> 7 (se permitir_mover_semanal=True).
    """
    realocados = []
    for semana_id, dias in agenda.items():
        # Calcula cargas e metas
        ...
        # Alvo: não exceder capacidade e aproximar da média (até o máximo capacidade)
        alvo_por_dia = min(capacidade_por_dia, max(media, 0))

        # Funções utilitárias
        def lista_moviveis(lst):
            # ordena por frequência: 30 primeiro, depois 14, depois 7
        ...
        # Loop de balanceamento (limite de iterações para evitar ciclos)
        for _ in range(1000):
            # identifica excedentes e deficit
            ...
            for origem in dias_excesso:
                # lista de candidatos a mover
                candidatos = lista_moviveis(dias[origem])
                for cliente in candidatos:
                    if not pode_mover(cliente):
                        continue
                    # destinos ordenados pelo déficit (mais vazios primeiro)
                    destinos_ordenados = sorted(
                        dias_deficit, key=lambda d: cargas[d]
                    )
            ...
            if not movido_na_iteracao:
                # não foi possível mover mais nada
                break
...
with colp[1]:
    capacidade = st.number_input("Capacidade por dia", min_value=10, max_value=200,
                                 value=st.session_state.parametros.get("capacidade_por_dia", 40))
with colp[2]:
    semanas = st.slider("Semanas no calendário", 1, 12, value=st.session_state.parametros.get("semanas", 6))
with colp[3]:
    inicio = st.date_input("Data inicial", value=st.session_state.parametros.get("inicio", datetime.date.today()))
with colp[4]:
    limiar_dia_pct = st.slider("Limiar desequilíbrio/dia (%)", 10, 90,
                               value=int(st.session_state.parametros.get("limiar_desequilibrio_porcento", 0.5) * 100)) / 100.0
with colp[5]:
    limiar_semana_pct = st.slider("Limiar desequilíbrio semanal (%)", 10, 90,
                                  value=int(st.session_state.parametros.get("limiar_semana_porcento", 0.3) * 100)) / 100.0
...
arquivo = st.file_uploader("Selecione o arquivo com colunas: nome, rota, dia_semana, frequencia", type=["xlsx"])
        ...
        if not colunas_ok:
            ...
            for _, row in df.iterrows():
                ...
                novo = {
                    ...
                    "rota": str(row["rota"]).strip(),
                    "dia_semana": str(row["dia_semana"]).strip(),
...
# Dropdown para escolher a semana
semanas_disponiveis = list(agenda.keys())
semana_escolhida = st.selectbox("Selecione a semana para visualizar", semanas_disponiveis)
...
if semana_escolhida:
    ...
    for dia in DIAS_SEMANA:
        ...
        dados.append({
            "Dia": dia,
            "Clientes": total,          # total de clientes no dia
            "Frequência 7": freq7,      # clientes semanais
            "Frequência 14": freq14,    # clientes quinzenais
            "Frequência 30": freq30,    # clientes mensais
        })
...
permitir_mover_semanal = st.checkbox("Permitir mover clientes semanais (007)", value=False)
preservar_dia = st.checkbox("Preservar dia original do cliente (não mover Segunda→Terça)", value=False)
executar = st.button("Executar redistribuição")

agenda_ajustada = None
realocados = []

if executar:
    # Cópia superficial da agenda
